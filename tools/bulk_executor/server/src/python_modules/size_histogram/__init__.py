...
class DictAccumulator(AccumulatorParam):
    """Accumulator that merges dicts by summing values for matching keys."""
...
def _item_size_bytes(item):
    """Calculate the DynamoDB-marshalled size of an item in bytes.

    Approximation: sum of (attribute name length + value byte length).
    For strings, length is the UTF-8 encoded length.
    For numbers, length is approximated as the string representation length.
    For binary, length is the raw byte length.
    For other types (bool, null), fixed sizes are used.
    """
    total = 0
    for attr_name, attr_value in item.items():
        ...
        total += _value_size(attr_value)
    return total


def _value_size(value):
    """Estimate the byte size of a DynamoDB attribute value."""
    if value is None:
        return 1
    ...
    if isinstance(value, dict):
        ...
        for k, v in value.items():
            size += len(k.encode('utf-8')) + _value_size(v)
        return size
...
def _size_to_bucket(size_bytes):
    """Map a size in bytes to a KB bucket index. Bucket 0 = 0-1KB, 1 = 1-2KB, etc."""
...
def run(job, spark_context, glue_context, parsed_args):
    ...
    print(f"\nSize Histogram ({total_items:,} items analyzed):")
    print("-" * 40)
...
def _size_data(monitor_options, table_name, index_name,
               ...
               histogram_accumulator, total_items_accumulator, error_accumulator,
               rate_limiter_shared_config):
    """Worker function: scans a segment, computes item sizes, and accumulates bucket counts."""
